...
int g_dev_major = FDDEV_MAJOR;
int g_dev_minor = FDDEV_MINOR;
...
struct clk *fd_moduleclk = NULL;
struct clk *fd_pll11clk = NULL;
struct clk *ahb_fdclk = NULL;
struct clk *dram_fdclk = NULL;
struct clk *hosc_clk = NULL;
struct clk *vpu_power_gating = NULL;
...
static irqreturn_t fdEngineInterupt(int irq, void *dev)
{
	 ...
	 writel(0x01,iomap_addrs.regs_fd + FD_STATUS_IOADDR);
...
int enable_fd_hw_clk(void)
{
	...
	//printk("FD_CLK_REG = %x\n",readl(FD_CLK_REG));
	//printk("PLL0_REG0 = %x\n",readl(PLL0_REG0));
	//printk("BUS_CLK_GATING_REG0 = %x\n",readl(BUS_CLK_GATING_REG0));
out:
	spin_unlock_irqrestore(&fd_spin_lock, flags);
	return res;
}

int disable_fd_hw_clk(void)
{
	unsigned long flags;
	int res = -EFAULT;
...
void clear_mode_context(int mode_idx)
{
	clear_angle_context();
...
long fddev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	long   ret = 0;
	struct fd_dev *devp;
	int fd_timeout;
	...
	switch(cmd)
	{
	   ...
	   case IOCTL_WRITE_REG    :
	   	  {
	   	     ...
	   	     if(copy_from_user(&reg_para, (void __user*)arg, sizeof(struct reg_paras)))
	   	  	...
	   	  	writel(reg_para.value,fd_devp->iomap_addrs.regs_fd + reg_para.addr);
	   		}
	   		break;
	   case IOCTL_READ_REG    :
	   	  {
	   	     ...
	   	     if(copy_from_user(&reg_para, (void __user*)arg, sizeof(struct reg_paras)))
	   	  	...
	   	  	return readl(fd_devp->iomap_addrs.regs_fd + reg_para.addr);
	   		}
	   		break;
	   case IOCTL_SET_MODE        :
	   {
	   ...
	   case IOCTL_BASE_GET_RTL    :
	   	  base_get_rtl(devp->iomap_addrs,devp->mem_addr,(unsigned char*)arg,1);
	   		break;
	   ...
	   case IOCTL_BASE_WAIT_ENGINE:
        ...
        wait_event_interruptible_timeout(wait_fd, devp->fd_irq_flag, fd_timeout*HZ);            
	      ...
	      {
	      	if(((readl(devp->iomap_addrs.regs_fd + FD_SYC_FLAG_IOADDR)&0x0FF) == devp->mem_addr.mem_sys_flag_addr[15])
...
void set_wbit(unsigned int addr,unsigned int  mask)
{
...
void clr_wbit(unsigned int addr,unsigned int  mask)
{
	unsigned int tmp = readl(addr);
	mask = ~mask;
	tmp &= mask;
...
int request_fd_resource(void)
{
    ...
    fd_devp->iomap_addrs.regs_fd = ioremap(FD_REGS_BASE, FD_REGS_SIZE);
    if (!fd_devp->iomap_addrs.regs_fd){
        printk("cannot map region for macc");
    }

   fd_devp->iomap_addrs.regs_ccmu = ioremap(CCMU_REGS_BASE, 1*1024);
	...
	fd_devp->phy_mem_addr.phy_mem_ld3_addr = sunxi_mem_alloc(FTR_FILE_SIZE);
	fd_devp->phy_mem_addr.phy_mem_ld4_addr = sunxi_mem_alloc(FTR_FILE_SIZE);
	...
	fd_devp->mem_addr.mem_ld3_addr = sunxi_map_kernel(fd_devp->phy_mem_addr.phy_mem_ld3_addr, FTR_FILE_SIZE);
	fd_devp->mem_addr.mem_ld4_addr = sunxi_map_kernel(fd_devp->phy_mem_addr.phy_mem_ld4_addr, FTR_FILE_SIZE);
	...
	if(fd_devp->mem_addr.mem_ld0_addr == 0)
	{
		printk("warning:map mem_ld0_addr fail\n");
		return -1;
	}
	if(fd_devp->mem_addr.mem_ld1_addr == 0)
	{
		printk("warning:map mem_ld1_addr fail\n");
		return -1;
	}
	if(fd_devp->mem_addr.mem_ld2_addr == 0)
	{
		printk("warning:map mem_ld2_addr fail\n");
		return -1;
	}
	if(fd_devp->mem_addr.mem_ld3_addr == 0)
	{
		printk("warning:map mem_ld3_addr fail\n");
		return -1;
	}
	if(fd_devp->mem_addr.mem_ld4_addr == 0)
	{
		printk("warning:map mem_ld4_addr fail\n");
		return -1;
	}
	if(fd_devp->mem_addr.mem_ld5_addr == 0)
	{
		printk("warning:map mem_ld5_addr fail\n");
		return -1;
	}
	if(fd_devp->mem_addr.mem_ld6_addr == 0)
	{
		printk("warning:map mem_ld6_addr fail\n");
		return -1;
	}
	if(fd_devp->mem_addr.mem_ld7_addr == 0)
	{
	...
	if(fd_devp->phy_mem_addr.phy_mem_rtl_addr == 0)
	{
		printk("warning:alloc phy_mem_rtl_addr fail\n");
		return -1;
	}
	
	fd_devp->mem_addr.mem_rtl_addr = sunxi_map_kernel(fd_devp->phy_mem_addr.phy_mem_rtl_addr,RTL_BUFFER_SIZE);
	if(fd_devp->mem_addr.mem_rtl_addr == 0)
	{
		printk("warning:map mem_rtl_addr fail\n");
		return -1;
	}
	
	fd_devp->phy_mem_addr.phy_mem_rtl_size = RTL_BUFFER_SIZE;
	
		//ftr config buffer,1024���룬8KByte
	fd_devp->phy_mem_addr.phy_mem_ftr_cfg_addr = sunxi_mem_alloc(FTR_CFG__BUFFER_SIZE);
	if(fd_devp->phy_mem_addr.phy_mem_ftr_cfg_addr == 0)
	{
		printk("warning:alloc phy_mem_ftr_cfg_addr fail\n");
		return -1;
	}
	fd_devp->mem_addr.mem_ftr_cfg_addr = sunxi_map_kernel(fd_devp->phy_mem_addr.phy_mem_ftr_cfg_addr,FTR_CFG__BUFFER_SIZE);
	if(fd_devp->mem_addr.mem_ftr_cfg_addr == 0)
	{
		printk("warning:map mem_ftr_cfg_addr fail\n");
		return -1;
	}
	fd_devp->phy_mem_addr.phy_mem_ftr_cfg_size = FTR_CFG__BUFFER_SIZE;
	
	//scale image buffer,�ݶ�6M,��Ҫ���ݼ��Ĳ���������16Byte����
	fd_devp->phy_mem_addr.phy_mem_scale_img_addr = sunxi_mem_alloc(SCALE_IMG_BUFFER_SIZE); 
	if(fd_devp->phy_mem_addr.phy_mem_scale_img_addr == 0)
	{
		printk("warning:alloc phy_mem_scale_img_addr fail\n");
		return -1;
	}
	fd_devp->phy_mem_addr.phy_mem_scale_img_size = SCALE_IMG_BUFFER_SIZE; 
	
	fd_devp->mem_addr.mem_scale_img_addr = sunxi_map_kernel(fd_devp->phy_mem_addr.phy_mem_scale_img_addr,fd_devp->phy_mem_addr.phy_mem_scale_img_size);
	if(fd_devp->mem_addr.mem_scale_img_addr == 0)
	{
		printk("warning:map mem_scale_img_addr fail\n");
		return -1;
	}
	
	//temp buffer,�ݶ�16M + 1KByte,��Ҫ���ݼ��Ĳ���������1024Byte����
	fd_devp->phy_mem_addr.phy_mem_tmpbuf_addr = sunxi_mem_alloc(TMP_BUFFER_SIZE);
	if(fd_devp->phy_mem_addr.phy_mem_tmpbuf_addr == 0)
	{
		fd_devp->phy_mem_addr.phy_mem_tmpbuf_addr = sunxi_mem_alloc(HALF_TMP_BUFFER_SIZE);
	  if(fd_devp->phy_mem_addr.phy_mem_tmpbuf_addr == 0)
	  {
	  	printk("warning:alloc phy_mem_tmpbuf_addr fail\n");
	  	return -1;
	  }
	  else
	  {
	...
	fd_devp->mem_addr.mem_tmpbuf_addr = sunxi_map_kernel(fd_devp->phy_mem_addr.phy_mem_tmpbuf_addr,fd_devp->phy_mem_addr.phy_mem_tmpbuf_size);
	if(fd_devp->mem_addr.mem_tmpbuf_addr == 0)
	{
		printk("warning:map mem_tmpbuf_addr fail\n");
		return -1;
	}
	
	
	//roi config address,�ݶ�1KByte,��Ҫ���ݼ��Ĳ�������,1024Byte����
	//fd_devp->mem_addr.mem_roi_addr = kmalloc(1*1024, GFP_KERNEL);
	fd_devp->phy_mem_addr.phy_mem_roi_addr = sunxi_mem_alloc(ROI_CFG_BUFFER_SIZE);
	if(fd_devp->phy_mem_addr.phy_mem_roi_addr == 0)
	{
		printk("warning:alloc phy_mem_roi_addr fail\n");
		return -1;
	}
	fd_devp->mem_addr.mem_roi_addr = sunxi_map_kernel(fd_devp->phy_mem_addr.phy_mem_roi_addr,ROI_CFG_BUFFER_SIZE);
	if(fd_devp->mem_addr.mem_roi_addr == 0)
	{
		printk("warning:map mem_roi_addr fail\n");
		return -1;
	}
	fd_devp->phy_mem_addr.phy_mem_roi_size = ROI_CFG_BUFFER_SIZE;
	//ͬ����־��ַ��16Byte,16Byte����
	//fd_devp->mem_addr.mem_sys_flag_addr = kmalloc(16, GFP_KERNEL);
	fd_devp->phy_mem_addr.phy_mem_sys_flag_addr = sunxi_mem_alloc(SYS_FLAG_BUFFER_SIZE);
	if(fd_devp->phy_mem_addr.phy_mem_sys_flag_addr == 0)
	{
		printk("warning:alloc phy_mem_sys_flag_addr fail\n");
		return -1;
	}
	
	fd_devp->mem_addr.mem_sys_flag_addr = sunxi_map_kernel(fd_devp->phy_mem_addr.phy_mem_sys_flag_addr,SYS_FLAG_BUFFER_SIZE);
	if(fd_devp->mem_addr.mem_sys_flag_addr == 0)
	{
		printk("warning:map mem_sys_flag_addr fail\n");
		return -1;
	}
...
//	unsigned long flags; 
   ...
   free_irq(SUNXI_IRQ_FD, NULL);
	iounmap(fd_devp->iomap_addrs.regs_fd);
	iounmap(fd_devp->iomap_addrs.regs_ccmu);
	...
	sunxi_unmap_kernel(fd_devp->mem_addr.mem_ld3_addr);
	sunxi_unmap_kernel(fd_devp->mem_addr.mem_ld4_addr);
	sunxi_unmap_kernel(fd_devp->mem_addr.mem_ld5_addr);
	sunxi_unmap_kernel(fd_devp->mem_addr.mem_ld6_addr);
	sunxi_unmap_kernel(fd_devp->mem_addr.mem_ld7_addr);
	
	sunxi_unmap_kernel(fd_devp->mem_addr.mem_rtl_addr);
	sunxi_unmap_kernel(fd_devp->mem_addr.mem_ftr_cfg_addr);
	sunxi_unmap_kernel(fd_devp->mem_addr.mem_scale_img_addr);
	sunxi_unmap_kernel(fd_devp->mem_addr.mem_tmpbuf_addr);
	sunxi_unmap_kernel(fd_devp->mem_addr.mem_roi_addr);
	sunxi_unmap_kernel(fd_devp->mem_addr.mem_sys_flag_addr);
	
	
	sunxi_mem_free(fd_devp->phy_mem_addr.phy_mem_ld0_addr,fd_devp->phy_mem_addr.phy_mem_ld0_size);
	sunxi_mem_free(fd_devp->phy_mem_addr.phy_mem_ld1_addr,fd_devp->phy_mem_addr.phy_mem_ld1_size);
	sunxi_mem_free(fd_devp->phy_mem_addr.phy_mem_ld2_addr,fd_devp->phy_mem_addr.phy_mem_ld2_size);
	sunxi_mem_free(fd_devp->phy_mem_addr.phy_mem_ld3_addr,fd_devp->phy_mem_addr.phy_mem_ld3_size);
	sunxi_mem_free(fd_devp->phy_mem_addr.phy_mem_ld4_addr,fd_devp->phy_mem_addr.phy_mem_ld4_size);
	sunxi_mem_free(fd_devp->phy_mem_addr.phy_mem_ld5_addr,fd_devp->phy_mem_addr.phy_mem_ld5_size);
	sunxi_mem_free(fd_devp->phy_mem_addr.phy_mem_ld6_addr,fd_devp->phy_mem_addr.phy_mem_ld6_size);
	sunxi_mem_free(fd_devp->phy_mem_addr.phy_mem_ld7_addr,fd_devp->phy_mem_addr.phy_mem_ld7_size);
	                        
	sunxi_mem_free(fd_devp->phy_mem_addr.phy_mem_rtl_addr,fd_devp->phy_mem_addr.phy_mem_rtl_size);
	sunxi_mem_free(fd_devp->phy_mem_addr.phy_mem_ftr_cfg_addr,fd_devp->phy_mem_addr.phy_mem_ftr_cfg_size);
	sunxi_mem_free(fd_devp->phy_mem_addr.phy_mem_scale_img_addr,fd_devp->phy_mem_addr.phy_mem_scale_img_size);

...
)
{
	...
	{
		ODM_RT_TRACE(pDM_Odm, ODM_COMP_CFO_TRACKING, ODM_DBG_LOUD, ("odm_Write_CrystalCap: Use default setting.\n"));
...
)
{
	...
	int							CFO_ave_diff, Adjust_Xtal = 0;
	...
	else
	{
		...
		//4 1.6 Calculate Xtal offset
		if(pCfoTrack->bAdjust)
		{
			if(CFO_ave > CFO_TH_XTAL_LOW)
				Adjust_Xtal =  ((CFO_ave - CFO_TH_XTAL_LOW) >> 2) + 1;
			else if(CFO_ave < (-CFO_TH_XTAL_LOW))
				Adjust_Xtal =  ((CFO_ave + CFO_TH_XTAL_LOW) >> 2) - 1;

			ODM_RT_TRACE(pDM_Odm, ODM_COMP_CFO_TRACKING, ODM_DBG_LOUD, ("ODM_CfoTracking(): bAdjust = %d, Crystal cap offset = %d\n", pCfoTrack->bAdjust, Adjust_Xtal));
		}
		...
		if(Adjust_Xtal != 0)
		{
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_CFO_TRACKING, ODM_DBG_LOUD, ("ODM_CfoTracking(): New crystal cap = 0x%x \n", pCfoTrack->CrystalCap));
		}
